use coi::{container, injectable, Container, Provide, Result};
use std::sync::Arc;
...
impl Provide for Impl1Provider {
    type Output = dyn Trait1 + Send + Sync + 'static;

    fn provide(&self, _: &Container) -> Result<Arc<Self::Output>> {
        Ok(Arc::new(Impl1) as injectable!(dyn Trait1))
    }
}
...
impl Provide for Impl2Provider {
    type Output = dyn Trait2 + Send + Sync + 'static;

    fn provide(&self, container: &Container) -> Result<Arc<Self::Output>> {
        let trait1 = container.resolve::<dyn Trait1 + Send + Sync + 'static>("trait1")?;
        Ok(Arc::new(Impl2::new(trait1)) as injectable!(dyn Trait2))
    }
}

impl Impl2 {
    fn new(trait1: injectable!(dyn Trait1)) -> Self {
        Self { trait1 }
    }
}
