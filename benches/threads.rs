...
pub fn a_simple_resolve(c: &mut Criterion) {
    ...
    for _ in 0..5 {
        ...
        let handle = std::thread::spawn(move || loop {
            ...
            container
                .resolve::<dyn LoginService + Send + Sync>("login_svc")
                .unwrap();
    ...
    for _ in 0..5 {
        ...
        let handle = std::thread::spawn(move || loop {
            ...
            container.resolve::<Guid>("guid").unwrap();
            container
                .resolve::<dyn ShoppingCartService + Send + Sync>("shopping_cart_svc")
                .unwrap();
            if done.load(std::sync::atomic::Ordering::Acquire) {
                break;
            }
    ...
    for _ in 0..5 {
        ...
        let handle = std::thread::spawn(move || loop {
            ...
            container.resolve::<Guid>("guid").unwrap();
            container
                .resolve::<dyn AccountService + Send + Sync>("account_svc")
                .unwrap();
            if done.load(std::sync::atomic::Ordering::Acquire) {
                break;
            }
    ...
    c.bench_function("a simple threaded resolver", |b| {
        b.iter(|| {
            ...
            container.resolve::<Guid>("guid").unwrap();
            container
                .resolve::<dyn PaymentService + Send + Sync>("payment_svc")
                .unwrap();
        });
    });
